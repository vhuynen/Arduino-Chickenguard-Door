...
void setup()
{
    ...
    attachInterrupt(0, alarmIsr, LOW);
    attachInterrupt(1, alarmIsr, LOW);
...
void loop()
{
    if (alarmIsrWasCalled)
    {
        if (!digitalRead(3) )
        {
            ...
            printDateTime( RTC.get() );
            Serial.println(" --> Wake UP By Pin 3");
        }
        if (RTC.alarm(ALARM_2))
        {
            ...
            // set alarm 2 for every minute
            RTC.setAlarm(ALM2_MATCH_HOURS, 3, 5, 0, 0);    //daydate parameter should be between 1 and 7
            RTC.alarm(ALARM_2);                   //ensure RTC interrupt flag is cleared
            RTC.alarmInterrupt(ALARM_2, true);
        }
...
void sleepNow()         // here we put the arduino to sleep
{
    ...
    attachInterrupt(0, alarmIsr, LOW); // use interrupt 0 (pin 2) and run function
